...
class ContactGroups {
  ...
  static findById(id) {
    const sql = `SELECT * FROM GroupContact WHERE id = ?`;
  ...
  static delete(id) {
    try {
      const info = db.prepare(`DELETE FROM GroupContact WHERE id = ?`).run(id);
  ...
  static show() {
    try {
      const sql = `SELECT * FROM GroupContact`;
    ...
    try {
      const sql = `
        SELECT
          ...
          c.email
        FROM Contact c
        JOIN GroupContact gc ON c.id = gc.contactId
        JOIN Groups g ON gc.groupId = g.id
